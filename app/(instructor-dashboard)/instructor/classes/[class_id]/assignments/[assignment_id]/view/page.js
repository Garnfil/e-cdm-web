...
export default function ViewAssignmentPage() {
    ...
    const fetchAssignmentDetails = async (session) => {
        const response = await axios.get(`https://my-cdm.godesqsites.com/api/school-works/${assignment_id}`, {
            headers: {
                "Accept": "application/json",
                "Authorization": `Bearer ${session.token}`
            }
    ...
    const handleUpdateAssignment = async () => {
        try {
            const response = await axios.put(`https://my-cdm.godesqsites.com/api/assignments/${assignmentDetails.assignment_id}`, assignmentDetails, {
                headers: {
                    "Accept": "application/json",
                    "Authorization": `Bearer ${authSession.token}`
                }
