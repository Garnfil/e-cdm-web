...
const ClassChatPage = () => {
    ...
    const fetchClassMessages = async (session) => {
        try {
            const response = await axios.get(`http://192.168.100.44:8000/api/messages/classes/${params.class_id}`, {
                headers: {
    ...
    const fetchClass = async (session) => {
        try {
            const response = await axios.get(`http://192.168.100.44:8000/api/classes/${params.class_id}`, {
                headers: {
                    "Accept": "application/json",
                    "Authorization": `Bearer ${session.token}`
                }
    ...
    const handleSubmitMessage = async () => {
        try {
            ...
            console.log(data);
            const response = await axios.post(`http://192.168.100.44:8000/api/messages/classes/${params.class_id}`, data, {
                headers: {
                    Accept: "application/json",
