...
export default function ViewExamPage() {
    ...
    const fetchExamDetails = async (session) => {
        try {
            const response = await axios.get(`https://app-digital-cdm.godesqsites.com/api/school-works/${exam_id}`, {
                headers: {
                    "Accept": "application/json",
                    "Authorization": `Bearer ${session.token}`
                }
            ...
            setExamDetails({
                ...
                exam_id: response.data.school_work.exam.id,
                points: response.data.school_work.exam.points,
                assessment_type: response.data.school_work.exam.assessment_type,
                notes: response.data.school_work.exam.notes,
    ...
    const handleUpdateExam = async () => {
        try {
            const response = await axios.put(`https://app-digital-cdm.godesqsites.com/api/exams/${examDetails.exam_id}`, examDetails, {
                headers: {
                    "Accept": "application/json",
                    "Authorization": `Bearer ${authSession.token}`
                }
                ...
                <div>
                    ...
                    <nav className="breadcrumb" aria-label="Breadcrumb">
                        <ol className="list-none text-sm p-0 inline-flex">
                            ...
                            <li className="flex items-center">
                                <span className="mx-2">›</span>
                                <a href="#" className="font-bold">View Exam</a>
                            </li>
                        </ol>
                    </nav>
                </div>
                                ...
                                <div className='border border-black py-2 px-3 mb-5'>
                                    ...
                                    <div className='form-group'>
                                        ...
                                        <textarea className='form-control'
                                            ...
                                            name="description"
                                            onChange={handleChange}
                                            style={{ height: '200px' }} value={examDetails.description}></textarea>
                                    </div>
                                    <div className='my-3'>
                                        ...
                                        <div className='flex flex-col gap-3'>
                                            {
                                                examDetails.attachments.length > 0 ? (
                                                    examDetails.attachments.map(attachment => (
                                                ...
                                                <DialogContent className="bg-white">
                                                    <DialogHeader>
                                                        ...
                                                        <form className='my-3' method='POST' onSubmit={handleUploadAttachmentSubmit} encType='multipart/form-data'>
                                                            <input type='hidden' name='school_work_id' value={exam_id} />
                                                            <div className='form-group'>
                                        ...
                                        <div className="border border-black hover-shadow p-4 bg-white">
                                            <div className="flex flex-col gap-3">
                                                <div className="flex justify-between items-center">
                                                    ...
                                                    <div>
                                                        <h5 className="text-2xl">{selectedSubmittedWork.score}/{examDetails.points}</h5>
                                                    </div>
                                                </div>
                                                <div>
                                                    ...
                                                    <div className="grid grid-cols-2 gap-3 my-3">
                                                        {
                                                            selectedSubmittedWork?.attachments?.length > 0 ? (
                                                                selectedSubmittedWork?.attachments?.map(attachment => (
                                                                    <Link href={`/instructor/classes/${class_id}/school-works/${exam_id}/student-submissions/${attachment.student_submission_id}`} key={attachment.id}>
