...
export default function ViewExamPage() {
    ...
    const fetchExamDetails = async (session) => {
        try {
            const response = await axios.get(`http://192.168.100.44:8000/api/school-works/${exam_id}`, {
                headers: {
                    "Accept": "application/json",
                    "Authorization": `Bearer ${session.token}`
                }
    ...
    const handleUpdateExam = async () => {
        try {
            const response = await axios.put(`http://192.168.100.44:8000/api/exams/${examDetails.exam_id}`, examDetails, {
                headers: {
                    "Accept": "application/json",
                    "Authorization": `Bearer ${authSession.token}`
                }
