...
export default function ViewExamPage() {
    ...
    const fetchExamDetails = async (session) => {
        try {
            const response = await axios.get(`http://127.0.0.1:8000/api/school-works/${exam_id}`, {
                headers: {
                    "Accept": "application/json",
                    "Authorization": `Bearer ${session.token}`
                }
    ...
    const handleDeleteAttachment = async (attachment_id) => {
        withReactContent(Swal).fire({
            ...
            if (result.isConfirmed) {
                try {
                    const response = await axios.delete(`http://127.0.0.1:8000/api/school-works/attachments/${attachment_id}/destroy`, {
                        headers: {
                            "Accept": "application/json",
                            "Authorization": `Bearer ${authSession.token}`,
                        }
