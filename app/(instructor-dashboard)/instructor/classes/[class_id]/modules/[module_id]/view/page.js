...
export default function ViewModulePage() {
    ...
    const fetchModule = async (session) => {
        try {
            const response = await axios.get(`http://192.168.100.44:8000/api/modules/${module_id}`, {
                headers: {
    ...
    const handleUploadAttachmentSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.post(`http://192.168.100.44:8000/api/modules/attachments/single-upload`, formData, {
                headers: {
                    "Accept": "application/json",
    ...
    const handleUpdateModule = async () => {
        try {
            const response = await axios.put(`http://192.168.100.44:8000/api/modules/${moduleDetails.id}`, moduleDetails, {
                headers: {
                    "Accept": "application/json",
                    "Authorization": `Bearer ${authSession.token}`
                }
