...
export default function ViewActivityPage() {
    ...
    const fetchActivityDetails = async (session) => {
        try {
            const response = await axios.get(`https://e-learn.godesqsites.com/api/school-works/${activity_id}`, {
                headers: {
                    "Accept": "application/json",
                    "Authorization": `Bearer ${session.token}`
                }
    ...
    const handleUpdateActivity = async () => {
        try {
            const response = await axios.put(`https://e-learn.godesqsites.com/api/activities/${activityDetails.activity_id}`, activityDetails, {
                headers: {
                    "Accept": "application/json",
                    "Authorization": `Bearer ${authSession.token}`
                }
