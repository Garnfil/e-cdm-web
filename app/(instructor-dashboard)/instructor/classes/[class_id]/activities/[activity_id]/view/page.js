...
export default function ViewActivityPage() {
    ...
    const fetchActivityDetails = async (session) => {
        try {
            const response = await axios.get(`https://app-digital-cdm.godesqsites.com/api/school-works/${activity_id}`, {
                headers: {
                    "Accept": "application/json",
                    "Authorization": `Bearer ${session.token}`
                }
    ...
    const handleUpdateActivity = async () => {
        try {
            const response = await axios.put(`https://app-digital-cdm.godesqsites.com/api/activities/${activityDetails.activity_id}`, activityDetails, {
                headers: {
                    "Accept": "application/json",
                    "Authorization": `Bearer ${authSession.token}`
                }
