...
export default function ViewActivityPage() {
    ...
    const fetchActivityDetails = async (session) => {
        try {
            const response = await axios.get(`http://192.168.100.44:8000/api/school-works/${activity_id}`, {
                headers: {
                    "Accept": "application/json",
                    "Authorization": `Bearer ${session.token}`
                }
    ...
    const handleUpdateActivity = async () => {
        try {
            const response = await axios.put(`http://192.168.100.44:8000/api/activities/${activityDetails.activity_id}`, activityDetails, {
                headers: {
                    "Accept": "application/json",
                    "Authorization": `Bearer ${authSession.token}`
                }
