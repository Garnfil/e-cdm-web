...
export default function CreateClassPage() {
    ...
    const fetchSections = async (year_level = 1) => {
        const response = await axios.get(`http://127.0.0.1:8000/api/sections?year_level=${year_level}`, {
            headers: {
                'Accept': 'application/json',
    ...
    const handleSubmitBtnClicked = async () => {
        const response = await axios.post('http://127.0.0.1:8000/api/classes', classDetails, {
            headers: {
                'Accept': 'application/json',
                'Authorization': `Bearer ${authSession.token}`,
