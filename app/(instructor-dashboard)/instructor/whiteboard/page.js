...
export default function InstructorWhiteBoardPage() {
    ...
    const fetchInstructorWhiteboards = async (session) => {
        try {
            const response = await axios.get(`https://e-learn.godesqsites.com/api/whiteboards/instructors/${session?.user?.id}`, {
                headers: {
                    "Accept": "application/json",
                    "Authorization": `Bearer ${session?.token}`,
                }
    ...
    const handleCreateWhiteboard = async (e) => {
        ...
        try {
            ...
            const response = await axios.post(`https://e-learn.godesqsites.com/api/whiteboards/generate-room`, formData, {
                headers: {
                    "Accept": "application/json",
